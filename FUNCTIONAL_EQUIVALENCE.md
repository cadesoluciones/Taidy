...
| F-17 | Tareas programadas: create (interval/cron), any action incl. workflows | `app.py:page_schedules` (new section) | `scheduler.add_schedule` | role check (Admin) | writes `schedules.json`; registers APScheduler job | `POST /schedules` | unit (cron/interval validation), API | done |
