...
export function createWorkflow(name: string, steps: StepDefinition[], description = ""): Promise<Workflow> {
  return apiPost<Workflow>("/workflows", { name, description, steps });
}

export function updateWorkflow(id: string, name: string, steps: StepDefinition[], description = ""): Promise<Workflow> {
  return apiPatch<Workflow>(`/workflows/${id}`, { name, description, steps });
}
