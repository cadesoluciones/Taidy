...
import { uploadHubspot } from "../../api/tasks";
...
export function HubspotUploadPage() {
  const { user } = useAuth();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const task = await uploadHubspot({
  ...
  return (
    <section>
      <PageHeader title="Subir CSVs de HubSpot a Fabric OneLake" />
      ...
      <form className={formStyles.card} onSubmit={handleSubmit}>
        ...
        <div className={formStyles.field}>
          <div className={formStyles.labelRow}>
            <label htmlFor="tables">Objetos (vacío = todos)</label>
            {isAdmin && (
        ...
        <button type="submit" className={formStyles.submit} disabled={isSubmitting || isReader}>
          {isSubmitting ? "Ejecutando…" : "Ejecutar subida HubSpot"}
        </button>
      </form>
    </section>
  );
