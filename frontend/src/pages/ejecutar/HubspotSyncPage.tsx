...
import { syncHubspot } from "../../api/tasks";
...
export function HubspotSyncPage() {
  const { user } = useAuth();
  ...
  return (
    <section>
      <PageHeader title="Extraer + subir HubSpot en un paso" />
      ...
      <form className={formStyles.card} onSubmit={handleSubmit}>
        ...
        <button type="submit" className={formStyles.submit} disabled={isSubmitting || isReader}>
          {isSubmitting ? "Ejecutando…" : "Ejecutar sync HubSpot"}
        </button>
      </form>
    </section>
  );
