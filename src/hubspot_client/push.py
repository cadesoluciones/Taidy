# -*- coding: utf-8 -*-
"""
CLI for uploading HubSpot CRM CSV exports to Microsoft Fabric OneLake.

Full extraction only for now, so this only ever finds a full/ directory to
upload (no incremental/<run_ts>/ archives yet):
  raw/hubspot/full/<table>.csv
"""
...
def _load_settings(upload_dir: Path, *, overwrite: bool = True) -> FabricUploadSettings:
    ...
    source = section.get("source_name", "hubspot")
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Upload HubSpot CRM CSV exports to Fabric OneLake"
    )
    parser.add_argument(
        "--output-dir",
        default=DEFAULT_OUTPUT_DIR,
        help=f"Directory with HubSpot CSV exports (default: {DEFAULT_OUTPUT_DIR})",
    )
    parser.add_argument(
        "--tables",
        nargs="+",
        metavar="TABLE",
        help="Upload only these tables (e.g. --tables hubspot_contacts)",
    )
...
def run(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        ...
        logger.exception("HubSpot upload failed: %s", exc)
        return 1
